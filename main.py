...
def bicepCurl():
    ...
    counter = 0
    stage =None
    playsound("C:\Code\gym cam trainer\letsgo.mp3")
    ...
    with mp_pose.Pose(min_detection_confidence=0.8,min_tracking_confidence=0.8) as pose:  # setup mediapipe instance
        while cap.isOpened():     # taking current feed from webcam
            ...
            try:
                ...
                if angle > 160:
                    if(stage == 'up'):
                        playsound("C:\Code\gym cam trainer\go_up.mp3")
                        playsound('go_up.mp3',False)
                    stage = 'down'                   #flag
                    
                if angle < 30 and stage == 'down':
                    if(stage == 'down'):
                        playsound("C:\Code\gym cam trainer\go_down.mp3")
...
def shoulderPress():
    ...
    counter = 0
    stage =None
    playsound("C:\Code\gym cam trainer\letsgo_1.mp3")
    ...
    with mp_pose.Pose(min_detection_confidence=0.5,min_tracking_confidence=0.5) as pose:  # setup mediapipe instance
        while cap.isOpened():     # taking current feed from webcam
            ...
            try:
                ...
                landmarks = [1, 2, 3]  # Replace this with the appropriate definition for your use case
                ...
                cv2.putText(
                            image,
                            str(angle),
                            tuple(np.multiply(knee, [640, 480]).astype(int)),  # Coordinates
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (255, 255, 255),
                            2,
...
def clicked():
    ...
    pass
clicked = 42  # Replace 42 with the appropriate value
def show():
